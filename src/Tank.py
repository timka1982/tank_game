...
class Tank(MovingObject):
    ...
    def update(self):
        if self.state == TankState.TURNING and self.target_angle:
        ...
        if self.state == TankState.TURNING_WHILE_MOVING and self.target_angle:
            # self target angle is computed in is_change_direction in case of two keys being pressed
    ...
    def is_change_direction(self, keys_pressed):
        if len(keys_pressed) == 1 and self.orientation != keys_pressed[0]:
            current_key_direction = keys_pressed[0]
        ...
        # here check if one of the keys are the current orientation
            self.state = TankState.TURNING_WHILE_MOVING
            if any(key == self.orientation for key in keys_pressed):
                self.target_angle = (self.angle + 3) % 360
    ...
    def move(self, keys, dt):
        ...
        if self.state != TankState.TURNING:
            for key, (direction, vec) in self.direction_map.items():
                if keys[key]:
                    new_keys_list.append(direction)

            if new_keys_list and len(new_keys_list) <= 2:
                self.is_change_direction(new_keys_list)
